...
c. Download the file oakevict.csv from Canvas (Files - Week 6 - Assignment).  The file contains 2019 housing eviction rates evrate for census tracts in Oakland. Read this file into R and merge it into the object you created in (b). 
...
3. Your final project proposal is due in a few weeks. If you haven't already done so, now is a good time to start thinking about which community or communities you will be examining in the project. If you haven't already selected a city or county, browse the eligible list and select a potential candidate. Using data downloaded from the Census API, present a choropleth map of a characteristic for census tracts located in your community. Make sure the map is presentation ready. From the map, briefly summarize the geographic distribution of the characteristic.
